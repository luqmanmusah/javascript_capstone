...
const countMovies = async (film) => {
  const numMovies = film.length
  navCount.innerHTML = `Movies(${numMovies})`;
}

const init = async () => {
  await myMovies.getMovies();
  countMovies(myMovies.movieList);
